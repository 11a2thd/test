...
class Sort_byController{
    show(req, res){
        Promise.all([Product.find({}).lean(), Buy.findOne({ user_name: req.params.slug0 }).lean()])
        .then(([product, buy]) => {
            var pData = product
            var bData = []
    ...
    cart(req, res){
        Promise.all([Product.findOne({ _id: req.params._id }), Buy.findOne({ user_name: req.cookies.user_name })])
            .then(([product, buy]) => {
                ...
                .then(() => {
                    res.redirect('/sort_by/' + req.params.slug)
                })
...
module.exports = new Sort_byController